...
import { checkForOrbCollisions, checkForPlayerCollisions } from "./player/checkCollisions.js";

/**
 * Import Classes
 */
import Player from "./player/classes/player.js";
import PlayerConfig from "./player/classes/PlayerConfig.js";
import PlayerData from "./player/classes/PlayerData.js";
import Orb from "./player/classes/Orb.js";

// make an orbs array that will host all 500/5000 NOT PLAYER orbs.
//every time on is absorb, the server will make a new one
const orbs = [];
const players = [];
...
export function attachSocketServer(httpServer) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on('init', (playerObj, ackCallback) => {
            ...
            // make a playerConfig object - the data specify to this player that only the player needs to know
            const playerName = playerObj.playerName;
            const playerConfig = new PlayerConfig(settings);
            const playerData = new PlayerData(playerName, settings);
            player = new Player(socket.id, playerConfig, playerData);
            players.push(player);
            // make a playerData object - the data specific to this plater that everyone needs to know
            ...
            socket.on('tock', (data) => {
                // a tock has come in before the plater is set up
                ...
                const absorbData = checkForPlayerCollisions(player.playerData, player.playerConfig, playersForUsers, socket.id);
                if (absorbData) {
            ...
            socket.on('disconnect', (reason) => {
                ...
                for(let i = 0; i < players.length; i++) {
                    if (players[i].socketId === player.socketId) {
                        ...
                        // splice the plater out of the player AND playersForUsers
                        players.splice(i, 1, {})
